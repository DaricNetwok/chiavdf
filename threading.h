...
template<int d_expected_size, int d_padded_size>
struct alignas(64) mpz;
...
struct alignas(64) mpz {
    ...
    template<class mpz_b>
    void set_sub(uint64 a, const mpz_b &b) {
        mpz_ui_sub(*this, a, b);
    }
    ...
    template<int size>
    array<uint64, size> to_array() const {
...
template<bool is_write, class type>
void prefetch(const type &p) {
...
template<class type>
void prefetch_write(const type &p) { prefetch<true>(p); }

template<class type>
void prefetch_read(const type &p) { prefetch<false>(p); }
...
struct alignas(64) gcd_uv_entry {
    ...
    void matrix_multiply(const mpz_type &in_a, const mpz_type &in_b, mpz_type &out_a, mpz_type &out_b) const {
...
template<class mpz_type>
struct alignas(64) gcd_results_type {
    mpz_type as[2];
    mpz_type bs[2];
